...
日志会生成在同文件夹中的logs.txt中（只有才运行了一次软件后日志才会生成，仅保留最后2000条），如果遇到有资产的地址你会在这个文件夹内看到一个non_zero_addresses.txt的文件，打开就能看到详细信息，如果non_zero_addresses.txt还没生成那就代表还没碰撞出来，默认使用TronGrid API动态查询地址，如果遇到403报错可以更换ip
