...
当发现任何余额不为0的钱包出现则会自动把钱包信息保存在本地，原理为在本地持续生成私钥和对应的地址，然后把生成的地址在链上查询是否有余额，如果是个有资产的钱包，那么恭喜你这个钱包属于你了！

日志会生成在同文件夹中的logs.txt中（运行过软件后日志才会生成，仅保留最后2000条），如果遇到有资产的地址你会在这个文件夹内看到一个non_zero_addresses.txt的文件，打开就能看到详细信息（包含该地址的私钥，导入到你的钱包就能直接使用了），如果non_zero_addresses.txt还没生成那就代表还没碰撞出来，默认使用TronGrid API动态查询地址，如果遇到403报错可以更换ip

此版本每秒效率15次，仅做演示功能，此版本不包含CUDA加速和多GPU兼容（因为不需要)

正式软件每秒效率亿为单位（包含CUDA加速和多GPU兼容，根据当前设备算力，例如3060显卡每秒可达二百多亿次）
